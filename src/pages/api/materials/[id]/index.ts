import type { NextApiRequest, NextApiResponse } from 'next';
import { roqClient } from 'server/roq';
import { prisma } from 'server/db';
import { errorHandlerMiddleware } from 'server/middlewares';
import { materialValidationSchema } from 'validationSchema/materials';
import { HttpMethod, convertMethodToOperation, convertQueryToPrismaUtil } from 'server/utils';
import { getServerSession } from '@roq/nextjs';

async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { roqUserId, user } = await getServerSession(req);
  await prisma.material
    .withAuthorization({
      roqUserId,
      tenantId: user.tenantId,
      roles: user.roles,
    })
    .hasAccess(req.query.id as string, convertMethodToOperation(req.method as HttpMethod));

  switch (req.method) {
    case 'GET':
      return getMaterialById();
    case 'PUT':
      return updateMaterialById();
    case 'DELETE':
      return deleteMaterialById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getMaterialById() {
    const data = await prisma.material.findFirst(convertQueryToPrismaUtil(req.query, 'material'));
    return res.status(200).json(data);
  }

  async function updateMaterialById() {
    await materialValidationSchema.validate(req.body);
    const data = await prisma.material.update({
      where: { id: req.query.id as string },
      data: {
        ...req.body,
      },
    });

    return res.status(200).json(data);
  }
  async function deleteMaterialById() {
    const data = await prisma.material.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
}

export default function apiHandler(req: NextApiRequest, res: NextApiResponse) {
  return errorHandlerMiddleware(handler)(req, res);
}
