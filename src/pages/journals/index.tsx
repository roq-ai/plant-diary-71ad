...
import { getJournals, deleteJournalById } from 'apiSdk/journals';
import { JournalInterface } from 'interfaces/journal';

type ColumnType = ColumnDef<JournalInterface, unknown>;
...
export function JournalListPage(props: JournalListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getJournals({
        ...
        order: params.order,
        searchTermKeys: ['entry.contains', 'tag.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<JournalInterface>>(
    () => `/journals?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: JournalInterface) => {
    if (hasAccess('journal', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/journals/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'entry', header: 'Entry', accessorKey: 'entry' },
    ...
    hasAccess('planting', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/plantings/view/${record.planting?.id}`}>
              {record.planting?.name}
            </Link>
          ),
        }
      : null,
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/journals/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('journal', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/journals/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('journal', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="journal" />
        </Flex>
        {hasAccess('journal', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/journals/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
